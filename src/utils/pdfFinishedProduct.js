...
export const pdfFinishedProduct = (Details) => {
  ...
  try {
    ...
    doc.text(
      "201/1/4, SR Compound, Dewas Naka,",
      pageWidth - 90,
      currentY + 15
    );
    doc.text(
      "Lasudia Mori, Indore, Madhya Pradesh 452016",
      pageWidth - 90,
      currentY + 22
    );
    ...
    currentY += 8;
    doc.text(
      `Customer Name: ${Details?.orderDetails?.customerName || "N/A"}`,
      marginLeft,
      currentY
    );
    doc.text(
      `Order Date: ${
        Details?.orderDetails?.createdAt
    ...
    doc.autoTable({
      ...
      body: [
        [
          Details?.productionManagerDetails?.production_details?.roll_size ||
            "N/A",
          Details?.productionManagerDetails?.production_details
            ?.cylinder_size || "N/A",
          Details?.productionManagerDetails?.production_details?.quantity_kgs ||
            "N/A",
          Details?.productionManagerDetails?.production_details?.progress ||
            "N/A",
        ],
      ],
      theme: "grid",
    ...
    doc.autoTable({
      ...
      body: [
        [
          ...
          Details?.deliveryDetails?.deliveryDate
            ? new Date(
                Details?.deliveryDetails?.deliveryDate
              ).toLocaleDateString()
            : "N/A",
    ...
    const subcategoryData = Details?.productionDetails?.subcategoryIds?.map(
      (sub) => [
        sub.fabricColor || "N/A",
        sub.rollSize || "N/A",
        sub.gsm || "N/A",
    ...
    doc.autoTable({
      ...
      body: [
        [`${subtotal.toFixed(2)}`, `${gst.toFixed(2)}`, `${total.toFixed(2)}`],
      ],
      theme: "grid",
      headStyles: {
        fillColor: [255, 87, 51],
